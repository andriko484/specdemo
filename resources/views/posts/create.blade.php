        ...
        <div class="px-4 py-5 sm:p-6">
            <form action="{{ route('posts.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="content" id="content" rows="5" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                    @enderror
                </div>
